...
const prefix = 'sy/' //البريفكس أو امر التشغيل
...
client.on('message', message => {
    ...
    if (message.content.startsWith(prefix + 'براول') || message.content.startsWith(prefix+"brawl")) {
      if(!message.channel.guild) return message.reply('**هذا الأمر للسيرفرات فقط**').then(m => m.delete(3000));
    
    const type = require('./Brawl.json');
    const item = type[Math.floor(Math.random() * type.length)];
    const filter = response => {
        return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
    };
    message.channel.send('**لديك __10__ ثانيه لحل السؤال**').then(msg => {
 const w = ['./img/w1.png'];//الخلفيه
            let Image = Canvas.Image,
            canvas = new Canvas(400, 150),
            ctx = canvas.getContext('2d');
...
message.channel.sendFile(canvas.toBuffer());
               ...
               maxMatches : 1,
                ...
                errors : ['time']
                      ...
                      .then((collected) => {
                           var embed = new Discord.RichEmbed()
                            .setDescription(`${collected.first().author} ✅ **احسنت لقد تمكنت من الإجابه على السؤال بسرعه**`)
                 message.channel.send(embed);
                  console.log(`[Typing] ${collected.first().author} typed the word.`);
                          let won = collected.first().author;
                          points[won.id].points++;
                        ...
                        .catch(collected => {
                       var embed1 = new Discord.RichEmbed()
                            .setDescription(`:x: **لم يتمكن احد من حل السؤال في الوقت المناسب**`)
                 message.channel.send(embed1);
                    console.log('[Typing] Error: No one type the word.');
...
client.on('message', message => {
    ...
    if (message.content.startsWith(prefix + 'حظ') || message.content.startsWith(prefix+"lucky")) {
      if(!message.channel.guild) return message.reply('**هذا الأمر للسيرفرات فقط**').then(m => m.delete(3000));
    
    const type = require('./lucky.json');
    const item = type[Math.floor(Math.random() * type.length)];
    const filter = response => {
        return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());
    };
   message.channel.send('**اختر رقم واحد بسرعة**').then(msg => {
 const w = ['./img/w1.png'];//الخلفيه
            let Image = Canvas.Image,
            canvas = new Canvas(400, 150),
            ctx = canvas.getContext('2d');
...
message.channel.sendFile(canvas.toBuffer());
               ...
               maxMatches : 1,
                ...
                errors : ['time']
                      ...
                      .then((collected) => {
                           var embed = new Discord.RichEmbed()
                            .setDescription(`${collected.first().author} ✅ **صحيح!**`)
                 message.channel.send(embed);
                  console.log(`[Typing] ${collected.first().author} typed the word.`);
                          let won = collected.first().author;
                          points[won.id].points++;
                        ...
                        .catch(collected => {
                       var embed1 = new Discord.RichEmbed()
                      .setDescription(`:x: **لقد خسرت :slight_frown: **`)
                 message.channel.send(embed1);
                    console.log('[Typing] Error: No one type the word.');
